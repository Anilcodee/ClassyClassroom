...
export default function ClassMessages() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      fetch(preview.url)
        ...
        .then((t) => {
          if (active)
            setPreview((prev) => (prev ? { ...prev, text: t } : prev));
        })
        .catch(() => {});
    }
    return () => {
  ...
  async function load(signal?: AbortSignal) {
    ...
    try {
      ...
      if (mountedRef.current) {
        ...
        const msgsFromOthers = (d.messages || []).filter(
          (m: any) => !userId || String(m.teacherId) !== String(userId),
        );
        const latest = msgsFromOthers.reduce(
          (acc: number, m: any) =>
            Math.max(acc, new Date(m.createdAt).getTime() || 0),
          0,
        );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const r = await fetchWithRetry(`/api/classes/${id}/assignments`, {
          headers,
          cache: "no-store",
          signal: ac.signal,
        });
        ...
        if (r.ok && mountedRef.current) {
          const latest = (d.assignments || []).reduce(
            (acc: number, a: any) =>
              Math.max(
                acc,
                new Date(a.publishAt || a.createdAt).getTime() || 0,
              ),
            0,
          );
  ...
  async function saveEdit(mid: string) {
    ...
    try {
          ...
          prev.map((m) =>
            m.id === mid
              ? {
                  ...
                  content: editContent,
                  attachments: [...(m.attachments || []), ...newAtts].slice(
                    0,
                    MAX_FILES,
                  ),
      ...
      const r = await fetch(`/api/messages/${mid}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: token ? `Bearer ${token}` : "",
  ...
  async function confirmDelete() {
    if (!deleteId) return;
    try {
      const r = await fetch(`/api/messages/${deleteId}`, {
        method: "DELETE",
    ...
    <main className="container mx-auto py-8">
      ...
      <div className="mt-2 flex items-center justify-between">
        ...
        <div
          ...
          onClick={() => {
            localStorage.setItem(`lastSeenAssigns:${id}`, String(Date.now()));
            setHasNewAssigns(false);
          }}
        >
      ...
      <div className="mt-4">
        ...
        >
          ...
          >
            <Button
              className="w-full justify-center gap-2 bg-blue-600 hover:bg-blue-600/90 text-white py-5 text-base"
              variant="default"
            >
                ...
                >
                  {editingId === m.id ? (
                    <>
                      ...
                      />
                      <textarea
                        className="w-full rounded-lg border border-input bg-background px-3 py-2 min-h-24"
                        value={editContent}
                  ...
                  {m.attachments && m.attachments.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-2">
                      {m.attachments.map((a, idx) => (
                        <div
                          key={idx}
                          className="inline-flex items-center gap-2 px-2 py-1 rounded-md border border-border text-xs hover:bg-accent"
                        >
                          <button
                            type="button"
                            onClick={() =>
                          ...
                          >
                            {a.type.startsWith("image/") ? (
                              ...
                              <span className="px-2 py-1 rounded bg-muted text-foreground/80">
                                File
                              </span>
                            )}
              ...
              <div className="mt-3 border-t border-border pt-3">
                ...
                {m.canComment ? (
                  ...
                  <p className="mt-2 text-xs text-foreground/60">
                    Only class members can comment. The poster cannot comment on
                    their own message.
                  </p>
                )}
              </div>
        ...
        >
          <div
            ...
            className={
              ...
              " overflow-auto bg-background border border-border"
            }
            ...
            <div className="p-3">
              ...
              {(preview.type.startsWith("text/") ||
                ...
                <pre
                  className={
                    (isFs ? "max-h-[90vh]" : "max-h-[70vh]") +
                    " w-full overflow-auto rounded bg-muted p-3 text-xs whitespace-pre-wrap break-all"
                  }
                >
              ...
              {preview.type.startsWith("video/") && (
                <video
                  controls
                  className={
                    (isFs ? "max-h-[90vh]" : "max-h-[70vh]") + " w-full"
                  }
                >
                  <source src={preview.url} />
                </video>
              )}
              {!preview.type.match(/^(image|audio|video)\//) &&
                !["application/pdf", "application/json"].includes(
                  preview.type,
                ) &&
                !preview.type.startsWith("text/") && (
