...
export const activateClass: RequestHandler = async (req: AuthRequest, res) => {
  ...
  try {
    ...
    if (cls.isActive && cls.activeSession) {
      const existing = await AttendanceSessionModelAny.findById(
        cls.activeSession,
      );
    ...
    const minutes = Math.max(
      1,
      Math.min(10, (cls as any).durationMinutes || 4),
    );
    const expiresAt = new Date(Date.now() + minutes * 60 * 1000);
    const session = await AttendanceSessionModelAny.create({
      classId: cls.id,
      expiresAt,
      isActive: true,
    });
...
export const sessionStatus: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    if (now > session.expiresAt && session.isActive) {
      ...
      await session.save();
      await ClassModelAny.updateOne(
        { _id: session.classId },
        { isActive: false, activeSession: null },
      );
    }
...
export const markAttendance: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    let attendance = await AttendanceModelAny.findOne({
      classId: session.classId,
      date: {
        $gte: new Date(key),
        $lt: new Date(new Date(key).getTime() + 24 * 60 * 60 * 1000),
      },
    });
    if (!attendance)
      attendance = await AttendanceModelAny.create({
        classId: session.classId,
        date: new Date(key),
        records: [],
      });
...
export const manualMarkAttendance: RequestHandler = async (
  req: AuthRequest,
  res,
) => {
  if (mongoose.connection.readyState !== 1)
  ...
  try {
    const { id } = req.params as { id: string };
    const { name, rollNo, present } = req.body as {
      name: string;
      rollNo: string;
      present: boolean;
    };
    ...
    let doc = await AttendanceModelAny.findOne({
      classId: id,
      date: { $gte: dayStart, $lt: dayEnd },
    });
    if (!doc)
      doc = await AttendanceModelAny.create({
        classId: id,
        date: dayStart,
        records: [],
      });
    ...
    const idx = doc.records.findIndex(
      (r) => String(r.student.rollNo) === String(rollNo),
    );
    if (present) {
      if (idx === -1) {
