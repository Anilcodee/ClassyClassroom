...
export const listMessages: RequestHandler = async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const enrolled = new Set(
      (user?.enrolledClasses || []).map((x: any) => String(x)),
    );
    const inRoster = (cls?.students || []).some(
      (s: any) => String(s.rollNo) === String((user as any)?.rollNo || ""),
    );
    const isMember =
    ...
    if (latestOnly) {
      ...
      return res.json({
        ...
        latestBy: latest?.teacherId || null,
        isMember,
      });
    }
...
export const createMessage: RequestHandler = async (req: AuthRequest, res) => {
  ...
  try {
    const { id } = req.params as { id: string };
    const { title, content, pinned, attachments } = req.body as {
      title?: string;
      content?: string;
      pinned?: boolean;
      attachments?: Array<{
...
export const updateMessage: RequestHandler = async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const { title, content, attachments } = req.body as {
      title?: string;
      content?: string;
      attachments?: Array<{
        name: string;
    ...
    if (typeof title === "string")
      update.$set = { ...(update.$set || {}), title: title || undefined };
...
export const addComment: RequestHandler = async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const isMember =
      isOwnerOrCo || enrolled.has(String(msg.classId)) || inRoster;
    if (!isMember)
      return res.status(403).json({ message: "Not a class member" });
