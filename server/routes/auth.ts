...
export const googleCallback: RequestHandler = async (req, res) => {
  try {
    ...
    if (stateRaw) {
      try { const st = JSON.parse(decodeURIComponent(stateRaw)); if (st?.role === "student") role = "student"; } catch {}
    }
    ...
    const origin = process.env.ORIGIN || `${req.protocol}://${req.get("host")}`;
    const redirectUri = `${origin}/api/auth/google/callback`;
    if (!clientId || !clientSecret) return res.status(500).send("Google client credentials not configured");
    ...
    if (!payload || !payload.email) return res.status(400).send("Failed to verify id token");
    const email = (payload.email || "").toLowerCase();
    const name = payload.name || "";

    const existing = await User.findOne({ email });
    // If existing user and already has the requested role, issue JWT and redirect with token
    const hasStudent = !!existing?.isStudent || (existing as any)?.role === "student";
    const hasTeacher = !!existing?.isTeacher || (existing as any)?.role === "teacher";
    const needsRole = role === "student" ? hasStudent : hasTeacher;

    if (existing && needsRole) {
      const token = jwt.sign({ id: existing.id }, process.env.JWT_SECRET || "dev-secret", { expiresIn: "7d" });
      const clientRedirect = role === "student" ? `${origin}/student-auth` : `${origin}/auth`;
      return res.redirect(`${clientRedirect}?token=${encodeURIComponent(token)}`);
    }
    ...
    const paramsOut = new URLSearchParams({ email, name, role, id_token: idToken });
...
export const googleComplete: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    if (existing) {
      ...
      await existing.save();
      const token = jwt.sign({ id: existing.id }, process.env.JWT_SECRET || "dev-secret", { expiresIn: "7d" });
      return res.json({ token, user: { id: existing.id, email: existing.email, name: existing.name, role: roleToUse, isTeacher: !!existing.isTeacher, isStudent: !!existing.isStudent, rollNo: existing.rollNo || null } });
    }
    ...
    const token = jwt.sign({ id: user.id }, process.env.JWT_SECRET || "dev-secret", { expiresIn: "7d" });
    res.json({ token, user: { id: user.id, email: user.email, name: user.name, role: roleToUse, isTeacher: !!user.isTeacher, isStudent: !!user.isStudent, rollNo: user.rollNo || null } });
  } catch (e: any) {
...
export const me: RequestHandler = async (req: any, res) => {
  try {
    ...
    res.json({ id: user.id, email: user.email, name: user.name, role: (user as any).role || (user.isStudent ? "student" : user.isTeacher ? "teacher" : "teacher"), isTeacher: !!user.isTeacher, isStudent: !!user.isStudent, rollNo: (user as any).rollNo || null });
  } catch (e: any) {
