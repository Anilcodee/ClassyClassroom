  ...
  try {
    ...
    try {
      ...
      if (published) {
        ...
        await MessageModel.create({ classId: id, teacherId: userId, title: doc.type === 'quiz' ? `Quiz: ${doc.title}` : `Assignment: ${doc.title}`, content: content || (doc.type === 'quiz' ? 'New quiz assigned.' : 'New assignment assigned.'), attachments: atts.slice(0, 5), comments: [], assignmentId: doc._id, assignmentPublishAt: doc.publishAt || null, assignmentDueAt: doc.dueAt || null });
      }
    } catch (e) {
  ...
  try {
    ...
    try {
      ...
      if (nowPublished && !wasPublished) {
        ...
        await MessageModel.create({ classId: a.classId, teacherId: (req as any).userId, title: a.type === 'quiz' ? `Quiz: ${a.title}` : `Assignment: ${a.title}`, content, attachments: atts.slice(0, 5), comments: [], assignmentId: a._id, assignmentPublishAt: a.publishAt || null, assignmentDueAt: a.dueAt || null });
      }
